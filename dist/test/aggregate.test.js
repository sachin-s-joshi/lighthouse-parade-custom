...
describe('aggregateCSVReports', () => {
    it('creates the expected csv', async () => {
        ...
        const expected = fs.readFileSync(path.join(dataPath, 'expectedAggregatedMobileReport.csv'));
        const data = fs.readFileSync(path.join(dataPath, 'aggregatedMobileReport.csv'));
    ...
    it('skips erroneous files', async () => {
        ...
        await aggregateCSVReports(dataPath);
        const data = fs.readFileSync(path.join(dataPath, 'aggregatedMobileReport.csv'), 'utf8');
