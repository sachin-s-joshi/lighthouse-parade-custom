...
sade('lighthouse-parade <url> [dataDirectory]', true)
    .version(version)
    .example('https://cloudfour.com --exclude-path-glob "/thinks/*" --max-crawl-depth 2')
    .describe('Crawls the site at the provided URL, recording the lighthouse scores for each URL found. The lighthouse data will be stored in the provided directory, which defaults to ./data/YYYY-MM-DDTTZ_HH_MM')
    .option('--ignore-robots', "Crawl pages even if they are listed in the site's robots.txt", false)
    .option('--crawler-user-agent', 'Pass a user agent string to be used by the crawler (not by Lighthouse)')
    .option('--lighthouse-concurrency', 'Control the maximum number of ligthhouse reports to run concurrently', os.cpus().length - 1)
    .option('--max-crawl-depth', 'Control the maximum depth of crawled links. 1 means only the entry page will be used. 2 means the entry page and any page linked directly from the entry page will be used.')
    .option('--include-path-glob', 'Specify a glob (in quotes) for paths to match. Links to non-matched paths will not be crawled. The entry page will be crawled regardless of this flag. This flag can be specified multiple times to allow multiple paths. `*` matches one url segment, `**` matches multiple segments. Trailing slashes are ignored.')
    .option('--exclude-path-glob', 'Specify a glob (in quotes) for paths to exclude. Links to matched paths will not be crawled. The entry page will be crawled regardless of this flag. This flag can be specified multiple times to exclude multiple paths. `*` matches one url segment, `**` matches multiple segments. Trailing slashes are ignored.')
    .action((url, 
// eslint-disable-next-line default-param-last
dataDirPath = path.join(process.cwd(), 'lighthouse-parade-data', usefulDirName()), opts) => {
    ...
    const excludePathGlob = toArray(opts['exclude-path-glob']).filter((glob) => glob !== undefined);
    ...
    const scanner = scan(url, {
        ...
        maxCrawlDepth,
        includePathGlob: includePathGlob,
        excludePathGlob: excludePathGlob,
    });
    const urlStates = new Map();
    const frames = ['⠋', '⠙', '⠹', '⠸', '⠼', '⠴', '⠦', '⠧', '⠇', '⠏'];
    let i = 0;
    const printLine = (url, state, error) => {
        const frame = kleur.blue(frames[i]);
        const statusIcon = error
            ? symbols.error
            : state === 0 /* State.Pending */
                ? ' '
                : state === 1 /* State.ReportInProgress */
                    ? frame
                    : symbols.success;
    ...
    const render = () => {
        ...
        urlStates.forEach(({ state, error }, url) => {
            if (state === 2 /* State.ReportComplete */)
                return;
        ...
        const numPendingToDisplay = Math.min(Math.max(process.stdout.rows - currentUrls.length - 3, 1), pendingUrls.length);
        const numHiddenUrls = numPendingToDisplay === pendingUrls.length
            ? ''
            : kleur.dim(`\n...And ${pendingUrls.length - numPendingToDisplay} more pending`);
        logUpdate(currentUrls.join('') +
     ...
     */
    const printAboveLogUpdate = (cb) => {
        logUpdate.clear();
        cb();
        render();
    };
    const log = (...messages) => printAboveLogUpdate(() => console.log(...messages));
    const warn = (...messages) => printAboveLogUpdate(() => console.log(...messages));
    ...
    scanner.on('urlFound', (url, contentType, bytes, statusCode) => {
        urlStates.set(url, { state: 0 /* State.Pending */ });
        const csvLine = [
            JSON.stringify(url),
            contentType,
            bytes,
    ...
    scanner.on('reportBegin', (url) => {
        urlStates.set(url, { state: 1 /* State.ReportInProgress */ });
    });
    scanner.on('reportFail', (url, error) => {
        urlStates.set(url, { state: 2 /* State.ReportComplete */, error });
        log(printLine(url, 2 /* State.ReportComplete */, error));
    });
    scanner.on('reportComplete', (url, reportData) => {
        urlStates.set(url, { state: 2 /* State.ReportComplete */ });
        log(printLine(url, 2 /* State.ReportComplete */));
